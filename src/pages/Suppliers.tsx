...
export default function Suppliers({ suppliers, purchaseInvoices, payments, onAddSupplier, onUpdateSupplier, onDeleteSupplier, onAddPayment }: Props) {
  ...
  const getBalance = (s: Supplier) => {
    ...
    const totalPaid = getSupplierInvoices(s.id).reduce((x, i) => x + i.paid, 0) + getSupplierPayments(s.id).reduce((x, p) => x + p.amount, 0);
  ...
  const printStatement = (s: Supplier) => {
    ...
    const allTrans = [
      ...
      ...pmts.map(p => ({ date: p.date, desc: `دفعة - ${p.paymentMethod === 'cash' ? 'كاش' : 'بنك'}`, debit: 0, credit: p.amount })),
          ...
          return (
            <div key={s.id} className="bg-[#1a1a35] border border-violet-900/30 rounded-2xl p-4 hover:border-violet-700/50 transition-all">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => printStatement(s)} className="py-1.5 px-2 text-xs bg-white/5 border border-white/10 rounded-xl text-gray-400 hover:text-white"><Printer size={12} /></button>
                <button onClick={() => openEdit(s)} className="py-1.5 px-2 text-xs bg-white/5 border border-white/10 rounded-xl text-gray-400 hover:text-violet-400">✏️</button>
              </div>
            </div>
          );
        })}
          ...
          <div className="bg-[#1a1a35] border border-violet-900/40 rounded-2xl p-6 w-full max-w-3xl my-4">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {getSupplierInvoices(viewSupplier.id).map(inv => (
                <div key={inv.id} className="bg-[#252545] rounded-xl p-3 flex items-center justify-between">
                  <div>
                    <div className="font-medium text-white text-sm">{inv.invoiceNumber}</div>
                  ...
                  <div className="text-right">
                    ...
                    {inv.remaining > 0 && <div className="text-xs text-red-400">متبقي: {formatCurrency(inv.remaining)}</div>}
                  </div>
                </div>
              ))}
