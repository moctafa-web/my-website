 ...
 *
 * On Windows the child is spawned through a shell (see `main`), so `child.kill`
 * only reaps the `cmd.exe` wrapper and leaves the real `vite`/`node` process
 * alive — still holding port 8080, which then fails the next `npm run dev`
 * with `Port 8080 is already in use`. `taskkill /T /F` kills the whole tree.
 */
